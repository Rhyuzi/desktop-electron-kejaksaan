...
export default {
	...
	methods: {
        ...
        socketConnect: function () {
            this.socket = io.connect(config.APP_DOMAIN, {
                reconnect: true,
                path: "/newsocket",
                query: {
                    connectType: "normal",
                    meetingNum: this.dataInfo.meetingNum,
                    sipAccount: this.dataInfo.sipAccount,
                }
            })
            // Get Participants
            this.socket.on(this.dataInfo.meetingNum + '-participants', (data, leave, join) => {
                if (typeof this.participantList === 'object') {
                    // Create Element
                    let first = new Set(this.participantList.map(d => d.sipAccount))
                    this.participants = [...this.participantList, ...data.filter(d => !first.has(d.sipAccount))]
                } else {
                    this.participants = data
                }
                this.participantList = data
                this.$store.dispatch('participants/hasLeaveMeeting', leave)
                this.participantList.map(item => this.$store.dispatch('participants/setParticipants', item))
                localStorage.setItem(`participants-${this.meetingInfos.meetingNum}`, JSON.stringify(data))
            })
            // Get Message
            this.socket.on(this.dataInfo.meetingNum + '-receiveMessage', (payload) => {
                console.warn('New Message', payload);
            })
            this.socket.on(this.dataInfo.meetingNum + '-receiveChat', (payload) => {
                console.warn('New Chat', payload);
            })
        },
